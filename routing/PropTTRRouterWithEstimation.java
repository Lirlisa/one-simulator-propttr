 ...
 * @version 1.0
 */
public class PropTTRRouterWithEstimation extends ActiveRouter {
	...
	/** PropTTR router's setting namespace ({@value})*/ 
	public static final String PROPTTR_NS = "PropTTRRouterWithEstimation";
	...
	*/
	 ...
	 */
	public PropTTRRouterWithEstimation(Settings settings) {
		super(settings);
                System.out.println("USANDO PropTTRWithEstimation");
		Settings propTTRSettings = new Settings(PROPTTR_NS);
		alpha = DEFAULT_ALPHA;
		timescale = propTTRSettings.getInt(TIME_SCALE_S);
		initMeetings();
	}
	 ...
	 */
	protected PropTTRRouterWithEstimation(PropTTRRouterWithEstimation r) {
		super(r);
		this.alpha = r.alpha;
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) { // new connection
			...
			if (con.isInitiator(getHost())) {
				...
				assert mRouter instanceof PropTTRRouterWithEstimation : "PropTTRWithEstimation only works "+ 
				" with other routers of same type";
				PropTTRRouterWithEstimation otherRouter = (PropTTRRouterWithEstimation)mRouter;
	...
	public MessageRouter replicate() {
		PropTTRRouterWithEstimation r = new PropTTRRouterWithEstimation(this);
		return r;
	}
}