 ...
 * be configured.
 */
public class MessageEventGeneratorWithCompression implements EventQueue {
	...
	protected Random rng;
        ...
        private final double[] compression_interval = {0.5, 0.9};
	 ...
	 */
	public MessageEventGeneratorWithCompression(Settings s){
	...
	protected int drawMessageSize() {
		int sizeDiff = sizeRange[0] == sizeRange[1] ? 0 : 
			rng.nextInt(sizeRange[1] - sizeRange[0]);
                double compression_factor = compression_interval[0] + rng.nextDouble() * (compression_interval[1] - compression_interval[0]);
		Double result = compression_factor*(sizeRange[0] + sizeDiff);
